...
function create_enemy()
  ...
  if(flag3>95)then
    enemy.width=player.width/4
    enemy.height=player.height/4
...
function enemy_draw()
  for keys,values in pairs(all_enemy)do
    if(values.color==power_color[3])then
      love.graphics.setColor(1,1,1)
      love.graphics.draw(enemy_image,values.x, values.y,nil,0.300)
    else
